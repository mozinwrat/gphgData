...
* Adds bounded multithreaded detail-page fetching with deterministic output.
"""
import argparse, os, re, sys, csv, itertools, requests, json, logging, html, threading, time
...
REQUEST_RETRIES = 3
DEFAULT_WORKERS = 6
USER_AGENT = "Mozilla/5.0 (compatible; GPHG-SuperScraper/81)"
...
def parse_args():
    ...
    p.add_argument("--workers", type=int, default=DEFAULT_WORKERS,
                   help="parallel detail-page workers; use 1 for sequential detail fetching")
    p.add_argument("--rate-limit", type=float, default=0.0,
                   help="optional seconds to sleep before each detail-page request per worker")
    return p.parse_args()
...
def thread_session(retries: int) -> requests.Session:
...
def is_invalid_spec_value(value: str) -> bool:
    if not value:
        return True
    if re.search(r"\|\s*GPHG\s*$", value, re.I):
        return True
    return False
...
def extract_specs_from_next_html(raw_html: str) -> dict:
    ...
    for match in label_rx.finditer(text):
        ...
        if not is_invalid_spec_value(value):
...
def get_specs_threaded(url: str, timeout: int, retries: int,
                       detail_cache: dict, cache_lock: threading.Lock,
    ...
    specs = extract_specs(thread_session(retries), url, timeout)

    with cache_lock:
        existing = detail_cache.get(url)
        if existing is not None:
            return existing
        detail_cache[url] = specs
    return specs

def build_detail_row(specs: dict, url: str, cat: str, year: int, code: str) -> dict:
    row = {
        "Year": year,
...
def fetch_detail_record(index: int, url: str, cat: str, year: int, code: str,
                        args, detail_cache: dict,
                        cache_lock: threading.Lock) -> tuple[int, dict]:
    specs = get_specs_threaded(
        url,
        args.timeout,
        args.retries,
        detail_cache,
    ...
    return index, build_detail_row(specs, url, cat, year, code)

def fetch_detail_batch(links: list[tuple[str, str]], year: int, code: str,
                       args, detail_cache: dict,
                       cache_lock: threading.Lock) -> list[dict]:
    total = len(links)
    if not total:
        return []

    workers = max(1, min(args.workers, total))
    logging.info("Fetching %s %s detail pages with %s worker(s)...",
                 total, code, workers)

    if workers == 1:
        rows = []
        spin = itertools.cycle(["-", "\\", "|", "/"])
        for idx, (url, cat) in enumerate(links, 1):
            sys.stdout.write(f"\r {code} {idx}/{total} {next(spin)}")
            sys.stdout.flush()
            try:
                _idx, row = fetch_detail_record(
                    idx, url, cat, year, code, args, detail_cache, cache_lock)
            except Exception as exc:
                logging.warning("Failed detail page (%s): %s", url, exc)
                continue
            rows.append((_idx, row))
        sys.stdout.write("\r" + " " * 60 + "\r")
        return [row for _idx, row in sorted(rows, key=lambda item: item[0])]
    ...
    with ThreadPoolExecutor(max_workers=workers) as executor:
        ...
        for future in as_completed(futures):
            ...
            completed += 1
            sys.stdout.write(f"\r {code} {completed}/{total} {next(spin)}")
            sys.stdout.flush()
            try:
...
def main():
    ...
    if args.participants:
        ...
        try:
            ...
            p_links = []
        total = len(p_links)
        for row in fetch_detail_batch(
            p_links, args.participants_year, "P", args, detail_cache, cache_lock
        ):
            specs = row
    ...
    for year in years:
        ...
        for code, lst in details.items():
            for row in fetch_detail_batch(
                lst, year, code, args, detail_cache, cache_lock
            ):
                specs = row
                add_spec_keys(specs, all_spec_keys)
                all_rows.append(row)
